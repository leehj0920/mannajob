...
public class ProfileController {
	...
	// empl.jsp
	@GetMapping("/empl")
	public String EmplJoin(Model model, HttpSession session, RedirectAttributes rttr) {
		System.out.println(service.cheakEmpl(session.getAttribute("userId").toString()));
		if(service.cheakEmpl(session.getAttribute("userId").toString())) {
			rttr.addFlashAttribute("error", 10);
